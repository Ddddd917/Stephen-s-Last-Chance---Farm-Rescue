...
class UIManager {
    ...
    _createCropCard(crop) {
        ...
        name.innerHTML = `<span class="item-emoji">${crop.emoji}</span> ${crop.name}`
    ...
    _createSeedCard(seed) {
        ...
        const name = document.createElement('div');
        name.className = 'item-name-large';
        name.innerHTML = `<span class="item-emoji">${seed.emoji}</span> ${seed.name} Seed`;
        nameSection.appendChild(name);
        header.appendChild(nameSection);
    ...
    _createAnimalCard(animal) {
        ...
        const name = document.createElement('div');
        name.className = 'item-name-large';
        name.innerHTML = `<span class="item-emoji">${animal.emoji}</span> ${animal.name}`;
        nameSection.appendChild(name);
        header.appendChild(nameSection);
    ...
    _createShopItemCard(itemType, itemDef) {
        ...
        header.innerHTML = `
            <div class="shop-item-icon">${itemType === 'seed' ? this._getCropEmoji(itemDef.id) : this._getAnimalEmoji(itemDef.id)}</div>
    ...
    _createSellItemCard(itemType, item) {
        ...
        header.className = 'shop-item-header';
        header.innerHTML = `
            <div class="shop-item-icon">${itemType === 'crop' ? '🌾' : item.emoji}</div>
            <div class="shop-item-title">
    ...
    _getAnimalEmoji(animalType) {
        const animalDef = GAME_CONFIG.ANIMALS.find(a => a.id === animalType);
        return animalDef ? animalDef.emoji : '🐔';
    }
    /**
     * PRIVATE: _getCropEmoji
     * Gets the emoji for a given crop type
     *
     * @param {string} cropType - Crop type ID
     * @returns {string} Crop emoji
     */
    _getCropEmoji(cropType) {
        const cropDef = GAME_CONFIG.CROPS.find(c => c.id === cropType);
        return cropDef ? cropDef.emoji : '🌾'; // 默认返回小麦图标
    }
}
